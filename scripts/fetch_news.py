...
try:
    ...
    feedparser = None  # only required for live RSS fetching; --from-json works without it
...
MIN_ARTICLES        = 5    # live fetch aborts (leaving published page untouched) below this
...
def main() -> None:
    ...
    parser.add_argument("--from-json", type=Path, default=None,
                         help="Load a curated article list instead of live-fetching "
                              "(same dedup/classify/score/render pipeline)")
    ...
    if args.from_json:
        ...
        all_articles.extend(load_curated(args.from_json))
        print(f"  Total curated: {len(all_articles)}", flush=True)
    else:
        ...
        if len(all_articles) < MIN_ARTICLES:
            print(f"\n❌ Only {len(all_articles)} articles fetched (need {MIN_ARTICLES}+). "
                  "Leaving published page untouched.", file=sys.stderr)
            sys.exit(1)
