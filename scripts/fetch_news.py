...
RSS_FEEDS = [
    ...
    {"url": "https://thegradient.pub/rss/",
     "source": "The Gradient",      "color": "#34d399"},
    {"url": "https://simonwillison.net/atom/everything/",
     "source": "Simon Willison",    "color": "#f472b6"},
    {"url": "https://hnrss.org/frontpage?q=AI+OR+LLM+OR+%22language+model%22+OR+agent&points=50",
     "source": "HackerNews",        "color": "#f97316"},
]
...
CATEGORIES: Dict[str, Dict] = {
    "research": {
        ...
        "keywords": [
            ...
            "gradient", "experiment", "model performance", "breakthrough",
        ]
    },
    "agents": {
        ...
        "keywords": [
            ...
            "self-improv", "task complet", "action", "langgraph", "autogen", "crewai",
            "mcp", "model context", "tool calling", "vision agent",
        ]
    },
    "products": {
        ...
        "keywords": [
            ...
            "service", "plugin", "integrat", "chatgpt", "copilot", "assistant",
            "deploy", "ship", "publish",
        ]
    },
    "industry": {
        ...
        "keywords": [
            ...
            "valuat", "ipo", "lawsuit", "safety", "govern", "deepmind", "anthropic",
            "xai", "inflection", "mistral", "cohere", "databricks", "scale ai",
        ]
    },
    "open_source": {
        ...
        "keywords": [
            ...
            "lm studio", "gguf", "quantiz", "ggml", "vllm", "deepseek",
        ]
    }
}
...
UA = "Mozilla/5.0 (compatible; AI-Pulse-Newsletter/3.0; +https://github.com/oeway/ai-news-channel)"
...
def _find_text(el: ET.Element, *paths) -> str:
    """Try multiple XPath-ish tag paths, return first non-empty."""
    for path in paths:
        try:
...
def parse_feed(raw: str, source: str, color: str) -> List[Dict]:
    """Parse RSS 2.0 or Atom feed without feedparser."""
    articles: List[Dict] = []
    try:
        # Strip problematic XML declarations / encoding headers
        raw = re.sub(r"<\?xml[^?]*\?>", "", raw, count=1)
        root = ET.fromstring(raw)
    except ET.ParseError:
        # Try cleaning up common issues
        try:
            raw_clean = raw.encode("utf-8", "replace").decode("utf-8")
    ...
    if "atom" in tag or root.tag == "{http://www.w3.org/2005/Atom}feed":
        ...
        for entry in entries[:25]:
            ...
            desc = _strip_tags(summ_el.text or "") if summ_el is not None else ""
            ...
            dt = to_dt(pub_el.text if pub_el is not None else None)

            if not title or not url:
                continue
            articles.append({"title": title, "url": url.strip(),
                              "desc": desc[:400], "source": source,
                              "source_color": color, "date": dt, "category": None})
    else:
        ...
        for item in items[:25]:
            ...
            # Description / content
            desc = ""
            for tag_name in ("content:encoded", "{http://purl.org/rss/1.0/modules/content/}encoded",
                             "description", "summary"):
                el = item.find(tag_name)
                if el is None:
                    el = item.find(tag_name, NS)
                if el is not None and el.text:
                    desc = _strip_tags(el.text)[:400]
                    break

            pub  = _find_text(item, "pubDate", "dc:date",
...
def fetch_rss(cfg: Dict) -> List[Dict]:
    ...
    if not raw:
        return []
    arts = parse_feed(raw, cfg["source"], cfg["color"])
...
def fetch_arxiv() -> List[Dict]:
    ...
    try:
        ...
        for entry in root.findall("a:entry", ns):
            ...
            cat_s   = " · ".join(cats[:3])
            auth_s  = ", ".join(authors) if authors else ""
            desc    = f"[{cat_s}]{' · ' + auth_s if auth_s else ''} — {summ}"
...
def score(a: Dict) -> float:
    ...
    if dt:
        ...
        s  += max(0, 12 - age * 0.07)  # decay over ~7 days
    source_bonus = {
        "arXiv": 3.0, "IEEE Spectrum": 2.2, "MIT Tech Review": 2.0,
        "The Gradient": 1.9, "Simon Willison": 1.8,
        "TechCrunch": 1.6, "VentureBeat": 1.4,
        "Wired": 1.4, "Hugging Face": 1.4,
...
def dedup(articles: List[Dict]) -> List[Dict]:
    ...
    for a in articles:
        ...
        tkey  = hashlib.md5(title_key.encode()).hexdigest()
...
body::before {
  ...
  background:
    radial-gradient(ellipse 65% 45% at 18%  8%,  rgba(99,102,241,.09) 0%, transparent 68%),
    radial-gradient(ellipse 55% 38% at 82% 88%,  rgba(6,182,212,.07)  0%, transparent 68%),
    radial-gradient(ellipse 38% 28% at 50% 50%,  rgba(139,92,246,.04) 0%, transparent 62%);
}
...
.site-header {
  ...
  background: linear-gradient(180deg, rgba(12,13,35,.98) 0%, rgba(6,7,26,.92) 100%);
...
.brand-logo { font-size: 1.8rem; line-height: 1 }
.brand-name {
...
.featured-title {
  ...
  font-size: clamp(1.4rem, 3.5vw, 2rem); font-weight: 700; line-height: 1.28;
...
.featured-body { color: var(--sub); line-height: 1.78; margin-bottom: 1.6rem; max-width: 720px; position: relative; z-index: 1 }
.featured-btn {
  display: inline-flex; align-items: center; gap: .4rem;
  background: linear-gradient(135deg, #6d28d9, #4f46e5); color: #fff;
  padding: .65rem 1.4rem; border-radius: 9px; font-size: .88rem; font-weight: 600;
...
.section-hdr {
  ...
  border-image: linear-gradient(90deg, color-mix(in srgb, var(--cat) 50%, transparent), transparent) 1;
}
...
.card::before {
  ...
  background: linear-gradient(90deg, var(--cat, #8b5cf6), color-mix(in srgb, var(--cat) 30%, transparent));
...
.card:hover {
  ...
  box-shadow: 0 10px 32px rgba(0,0,0,.4),
              0 0 0 1px color-mix(in srgb, var(--cat) 12%, transparent),
...
.card-desc {
  font-size: .8rem; color: var(--muted); line-height: 1.58; flex-grow: 1;
...
.footer-sources { display: flex; gap: .48rem; flex-wrap: wrap; justify-content: center; font-size: .73rem; color: var(--dim) }
.footer-sources span::after { content: '·'; margin-left: .48rem }
.footer-sources span:last-child::after { content: '' }
.footer-note { font-size: .73rem; color: var(--dim); line-height: 1.7 }
...
.scroll-top {
  ...
  background: rgba(12,13,35,.9); border: 1px solid rgba(167,139,250,.3);
...
def full_page(featured: Optional[Dict],
  ...
  <div class="empty-state">
    ...
    <p>No articles fetched yet. The GitHub Actions workflow will populate this on next run.</p>
  </div>'''
...
<head>
  ...
  <meta property="og:description" content="Daily curated AI news digest: research breakthroughs, AI agents, new products, and industry.">
  ...
  <header class="site-header">
    <div class="header-inner">
      ...
      <div class="header-right">
        ...
        <span class="header-date"><span class="live-dot"></span>{h(now_str)}</span>
      </div>
    </div>
    {nav_block}
  </header>
  ...
  <footer class="site-footer">
    <div class="footer-inner">
      ...
      <div class="footer-note">
        Auto-generated from {len(src_set)} sources · {total} articles · Updated {h(now_str)}<br>
        <time datetime="{gen_iso}">{gen_iso}</time> UTC
      </div>
    </div>
  </footer>
...
def main() -> None:
    ...
    # Pick featured: newest high-score article within 3 days
